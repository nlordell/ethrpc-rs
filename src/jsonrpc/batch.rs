...
where
    ...
    E: From<Error> + From<jsonrpc::Error> + From<serde_json::Error>,
{
...
{
    let (requests, body) = pre_call::<B, E>(batch)?;
    let body = roundtrip(body)?;
    post_call::<B, E>(requests, body)
}
...
{
    let (requests, body) = pre_call::<B, E>(batch)?;
    let body = roundtrip(body).await?;
    post_call::<B, E>(requests, body)
}

fn pre_call<B, E>(batch: B) -> Result<(Vec<Request>, String), E>
where
    B: Batch,
    E: From<Error> + From<serde_json::Error>,
{
...
fn post_call<B, E>(requests: Vec<Request>, body: String) -> Result<B::Results, E>
where
    B: Batch,
    E: From<Error> + From<serde_json::Error>,
{
    let mut responses = serde_json::from_str::<Vec<Response>>(&body)?;
    if responses.len() != requests.len() || responses.iter().any(|response| response.id.is_none()) {
...
mod tests {
    ...
    fn roundtrip(
        ...
        move |requests| {
            let requests = serde_json::from_str::<Vec<Request>>(&requests).unwrap();
            ...
            for (request, response) in requests.iter().zip(responses.as_array_mut().unwrap()) {
